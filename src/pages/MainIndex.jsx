...
export default function MainIndex() {
  ...
  const openBookings = useMemo(() => {
    return bookings.filter(b => {
      ...
      const isOpen = tempSt !== 'close' && tempSt !== 'closed' && st !== 'close' && st !== 'closed'
      if (!isOpen) return false
      if (form.customerName) {
        return (b.customerName || '').trim().toLowerCase() === (form.customerName || '').trim().toLowerCase()
      }
      return true
    })
  }, [bookings, form.customerName])

  const chosenVehicleCountOptions = useMemo(() => {
    return openBookings.map((b, idx) => {
      const count = b.customerVehicleCount || b.vehicleCount || (idx + 1)
      return `${count} (${b.serviceJobNo})`
    })
  }, [openBookings])

  const handleChosenVehicleCountSelect = (val) => {
    if (!val) {
      setForm(f => ({ ...f, chosenVehicleCount: '', vehicleCount: '', serviceJobNo: '' }))
      return
    }

    const matchedBooking = openBookings.find(b => {
      const optionLabel = `${b.customerVehicleCount || b.vehicleCount || 1} (${b.serviceJobNo})`
      return optionLabel === val || b.serviceJobNo === val || String(b.customerVehicleCount || b.vehicleCount) === val
    })

    if (matchedBooking) {
      handleServiceJobNoSelect(matchedBooking.serviceJobNo)
      setForm(f => ({
        ...f,
        chosenVehicleCount: val,
        vehicleCount: matchedBooking.customerVehicleCount || matchedBooking.vehicleCount || 1
      }))
    } else {
      setForm(f => ({ ...f, chosenVehicleCount: val, vehicleCount: val }))
    }
  }
            ...
            <div className="grid grid-cols-12 gap-x-12 gap-y-6">
              ...
              <div className="col-span-6 space-y-4">
                ...
                <div className="grid grid-cols-12 items-center gap-4">
                  ...
                  <div className="col-span-2">
                    <Select
                      options={chosenVehicleCountOptions}
                      placeholder="Select"
                      value={form.chosenVehicleCount || (form.serviceJobNo ? `${form.vehicleCount} (${form.serviceJobNo})` : '')}
                      onChange={e => handleChosenVehicleCountSelect(e.target.value)}
