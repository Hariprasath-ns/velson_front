...
import { useReferenceMaster, useCustomers, useEmployees, useServiceBookings } from '../hooks/useMasterData'
...
export default function MaterialIssue() {
  ...
  // References Data (React Query hooks)
  const { data: deptsData = [] } = useReferenceMaster('Department')
  const { data: vTypesData = [] } = useReferenceMaster('Vehicle_Type')
  const { data: empsRes = [] } = useEmployees()
  const { data: custsRes = [] } = useCustomers()
  const { data: bookingsRes = [] } = useServiceBookings()

  const departments = useMemo(() => deptsData.map(d => d.description).filter(Boolean), [deptsData])
  const models = useMemo(() => vTypesData.map(v => v.description).filter(Boolean), [vTypesData])
  const employees = empsRes
  const customers = custsRes
  const jobsList = bookingsRes
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const res = await api.get('/api/material-issue/next-number', { skipGlobalLoader: true })
      const nextNoRes = res.data?.data || {}
  ...
  const handleServicePartNoChange = async (val) => {
    ...
    try {
      ...
      // Fetch and set selectedPart details for servicePartNo so its image can be previewed
      const pNo = val.split(' - ')[0].trim()
      const res = await api.get(`/api/item-master?search=${encodeURIComponent(pNo)}&limit=1`, { skipGlobalLoader: true })
      const matchedItem = res.data?.data?.[0]
      if (matchedItem) {
        setSelectedPart(matchedItem)
      }
    } catch (err) {
      console.error(err)
  ...
  const handleSelectBomItem = async (item) => {
    ...
    try {
      const res = await api.get(`/api/item-master?search=${encodeURIComponent(item.partNo)}&limit=1`, { skipGlobalLoader: true })
      const matchedItem = res.data?.data?.[0]
      setSelectedPart(matchedItem || null)
      if (matchedItem) {
  ...
  const handlePartNoSelect = async (val, item) => {
    if (!item) {
      clearTransactionFields()
      return
    }
    ...
    setSpec(item.size || item.routeCardNo || '')
    setRate(item.purchaseRate || item.rate || 0)
    setSelectedPart(item)
    
    // Fetch barcodes & stocks
    setBarcodeSearch('')
    setSelectedBarcodeObj(null)
    setAvailableStock(0.0)
    try {
  ...
  const handlePartNameSelect = handlePartNoSelect;
  ...
  const partImageSrc = useMemo(() => {
    ...
    const matched = selectedPart && selectedPart.partNo === pNo ? selectedPart : null
    if (matched) {
      if (matched.hasImage) {
    ...
    return null
  }, [partNo, servicePartNo, selectedPart])
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <ItemSearchInput
                      value={partNo}
                      onChange={(val, item) => handlePartNoSelect(val, item)}
                      displayField="partNo"
                      placeholder="Search Part No"
                      className="w-full px-2.5 py-1 text-[13px] h-[32px] border border-slate-350 rounded bg-white text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-[#0097A7] focus:border-[#0097A7] transition-all duration-150 hover:border-slate-405"
                    />
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <ItemSearchInput
                      value={partName}
                      onChange={(val, item) => handlePartNameSelect(val, item)}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col">
                  ...
                  <div className="w-full h-[110px] mt-1.5 border border-slate-200 bg-slate-50 rounded-lg flex items-center justify-center overflow-hidden shadow-inner">
                    {partImageSrc ? (
                      <AuthenticatedImage src={partImageSrc} alt="Part" className="h-full w-full object-contain" />
                    ) : (
